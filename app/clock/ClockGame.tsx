...
function generateRounds(rng: Rng): ClockRound[] {
  return Array.from({ length: ROUNDS }, (_, i) => {
    const { far, minutePool, ...cfg } = roundConfig(i);
    ...
    return {
      ...cfg,
      reverse,
      duration: cfg.duration + (reverse ? 1 : 0),
      hour,
      minute,
...
export function ClockGame() {
  ...
  function startRun(m: Mode) {
    const generated = generateRounds(rngFor("clock", m));
...
function ClockFace({
  ...
  return (
    <svg
      ...
      className={
        mini
          ? "h-36 w-36 max-w-full sm:h-48 sm:w-48"
    ...
    >
      ...
      {/* Ticks — 12/3/6/9 get lemon anchors so the eye can orient instantly.
          Minis drop the minute ticks; they'd just be noise at that size. */}
      {Array.from({ length: 60 }, (_, i) => {
        ...
        return (
          <line
            ...
            strokeWidth={cardinal ? 4 : major ? 3 : 1.5}
      ...
      {hidden ? (
        ...
        <>
          {/* Hour hand: a short tapered baton — unmistakably the fat one */}
          <g ref={hourRef} style={handStyle}>
            <polygon
              points="95,104 95,76 100,68 105,76 105,104"
          ...
          {/* Minute hand: long and thin, arrowhead stops short of the digits */}
          <g ref={minuteRef} style={handStyle}>
            <line
              ...
              strokeLinecap="round"
            />
            <polygon points="100,48 94,64 106,64" fill="var(--color-lemon)" />
          </g>
